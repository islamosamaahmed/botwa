...
`.trim();
    ...
    try {
        if (/7eleven|foodshop|marketstall/i.test(command)) {
            // Tampilkan daftar makanan dan minuman tanpa memerlukan argumen
        ...
        if (/buyfood|buydrink/i.test(command)) {
            if (!type || !quantity) {
                return m.reply('Harap masukkan nama makanan/minuman dan jumlah yang valid.');
            }

            if (!foodPrices[type]) {
                return m.reply('Makanan atau minuman tidak tersedia.');
            }

            const foodPrice = foodPrices[type] * quantity;
            if (user.money < foodPrice) {
                return m.reply(`Uang anda kurang untuk membeli ${quantity} ${food[type]}.`);
            }

            user.money -= foodPrice;
            user[type] = (user[type] || 0) + quantity;
            return m.reply(`Anda baru saja membeli ${quantity} ${food[type]}.`);
        }
    } catch (err) {
        m.reply("Error\n\n\n" + err.stack);
    }
