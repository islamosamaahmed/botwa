const handler = async (m, { conn, usedPrefix }) => {
    ...
    if (new Date() - user.lastmancing> 3600000) {
        ...
        let hsl = `   
*R E S U L T S*
${ikan1 ? `🦀 Kepiting: ${ikan1}` : ''} ${ikan2 ? `🦞 Lobster: ${ikan2}` : ''} ${ikan3 ? `🦐 Udang: ${ikan3}` : ''} ${ikan4 ? `🦑 Cumi: ${ikan4}` : ''} ${ikan5 ? `🐙 Gurita: ${ikan5}` : ''} ${ikan6 ? `🐡 Buntal: ${ikan6}` : ''} ${ikan7 ? `🐠 Dory: ${ikan7}` : ''} ${ikan8 ? `🐳 Orca: ${ikan8}` : ''} ${ikan9 ? `🐬 Lumba: ${ikan9}` : ''} ${ikan10 ? `🐋 Paus: ${ikan10}` : ''} ${ikan11 ? `🦈 Hiu: ${ikan11}` : ''}
`.trim();
        user.kepiting += ikan1;
        user.lobster += ikan2;
        user.udang += ikan3;
