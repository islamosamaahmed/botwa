...
let handler = async (m, { conn, text, usedPrefix, command }) => {
  ...
  if (!text)
    throw `*• Example :* ${usedPrefix + command} *id Good Night*

*[ LIST LANGUAGE ]*
${await (await listLanguage()).map((a, i) => `> *${i + 1}* ${a.name.toUpperCase()} *[ ${a.code} ]*`).join("\n")}`;
  let keyword = text.split(" ")[0];
  let data = q.text.slice(keyword.length + 1);
  if (isCode.includes(keyword)) m.reply(wait);
  try {
    if (!isCode.includes(keyword))
      throw `*[ LIST LANGUAGE ]*
${await (await listLanguage()).map((a, i) => `> *${i + 1}* ${a.name.toUpperCase()} *[ ${a.code} ]*`).join("\n")}`;
    let txt = await (
      await translate(data, { to: keyword, autoCorrect: true }).catch(
        (_) => null,
      )
    ).text.toString();
    m.reply(txt);
