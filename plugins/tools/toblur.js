...
let handler = async (m, { conn, usedPrefix, command }) => {
  try {
    ...
    try {
      ...
      // request ke API blurwajah
      const response = await axios.get(
        `https://api-faa-skuarta2.vercel.app/faa/blurwajah?image=${encodeURIComponent(imageUrl)}`,
      ...
      await conn.sendMessage(
        m.chat,
        { image: imageBuffer, caption: "🤫 Nih wajahnya sudah diblur!" },
        { quoted: m }
      )
    } catch (err) {
      console.error(err)
...
handler.help = ["blurwajah", "toblur"].map(a => a + " *[reply/send media]*")
handler.tags = ["tools"]
handler.command = ["toblur", "blurwajah"]
handler.limit = true
