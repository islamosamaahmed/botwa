let handler = async (m, { conn, text, usedPrefix, command }) => {
  ...
  let number = no(Target) + "@s.whatsapp.net";
  let user = db.data.users[number];

  // kalau user belum ada → bikin default
  if (!user) {
    db.data.users[number] = {
      registered: false,
      banned: false,
      name: Target,
      age: "-",
  ...
  if (duration) {
    ...
    let hasil = Number(duration) * milidetikDalamHari;

    user.premium = true;
    user.premiumDate = Date.now() + hasil;
