...
const handler = async (m, { conn, command, text }) => {
  const querys = text.split(/\D+/).filter(Boolean),
    pluginsList = Object.keys(plugins).map((a) => a.replace(process.cwd(), "")),
    uniqueCategories = [];
      ...
      if (querys[0] && querys[1] && !isNaN(querys[0]) && !isNaN(querys[1])) {
        ...
        if (categoryIndex >= 1 && categoryIndex <= uniqueCategories.length) {
          const selectedCategory = uniqueCategories[categoryIndex - 1],
            pluginNames = pluginsList.filter(
              (plugin) => plugin.split("/")[2] === selectedCategory,
            );
      ...
      if (
        ...
        const selectedCategory = uniqueCategories[categoryIndex],
          pluginList = pluginsList
            .filter((plugin) => plugin.split("/")[2] === selectedCategory)
            .map(
              (plugin, index) =>
                `${index + 1}. *${plugin.split("/").pop().replace(".js", "")}*`,
            )
            .join("\n");
        m.reply(
          `*List File dalam Kategori "${selectedCategory}":*\n${pluginList}`,
        );
      } else m.reply("*Input salah* atau angka di luar rentang yang sesuai.");
    }
  else {
    const pluginList = uniqueCategories
