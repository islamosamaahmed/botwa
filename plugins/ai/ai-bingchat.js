      ...
      .then((res) => {
        if (res.status === 200) {
          const conversationSignature =
            res.headers["x-sydney-encryptedconversationsignature"] ?? "";
          return {
            ...res.data,
            conversationSignature,
          };
...
module.exports = {
  help: ["bingchat"].map((a) => a + " *[question]*"),
  tags: ["ai"],
  command: ["bingchat"],
  code: async (
    m,
    ...
    const res = await api.sendMessage(encodeURIComponent(text), {
      variant: "Balanced",
      messageType: "InternalSearchResult",
    });
    await conn.sendMessage(m.chat, { text: res.text, edit: key });
  },
};
