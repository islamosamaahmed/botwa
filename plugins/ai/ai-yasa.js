...
handler.help = ["yasa", "yasaai"].map((a) => a + " *[query]*");
handler.tags = ["ai"];
handler.command = ["yasa", "yasaai"];
handler.premium = true;
module.exports = handler;

async function GoogleBard(query) {
  return (await fetch(`https://itzpire.com/ai/yasa?q=${query}`)).json();
}

async function GoogleBardImg(query, url) {
  return (
    await fetch(`https://itzpire.com/ai/yasa?q=${query}?&url=${url}`)
  ).json();
}
