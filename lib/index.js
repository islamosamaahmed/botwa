...
async function isSudo(userId) {
    try {
        // Check if the user is the owner defined in settings.js
        // We compare the number part of the JID with the ownerNumber
        if (userId && userId.startsWith(settings.ownerNumber)) {
            return true;
        }

        // Then, check against the sudo list in the data file
        const data = loadUserGroupData();
