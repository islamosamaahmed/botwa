...
function detectArtworkNumber(input) {
  ...
  if (
    typeof input === "string" &&
    input.startsWith("https://www.pixiv.net/en/artworks/")
  ) {
...
const Pixiv = {
  search: async function fetchPixivSearchResults(word) {
    try {
      const response = await fetch(
        `http://api.obfs.dev/api/pixiv/search?word=${encodeURIComponent(word)}`,
      );
      const data = await response.json();
      return data.illusts;
    } catch (error) {
      console.log(
        "An error occurred while fetching Pixiv search results:",
        error,
      );
      throw error;
    }
  },
  id: async function fetchPixivIllust(id) {
    try {
      const response = await fetch(
        `http://api.obfs.dev/api/pixiv/illust?id=${id}`,
      );
      const data = await response.json();
      return data.illust;
    } catch (error) {
  ...
  random: async function vilipixRandomImg() {
    try {
      const response = await fetch(
